...
public:
    ...
    void bind_vertex_buffer(std::span<const VkBuffer> buffers, std::span<const VkDeviceSize> offsets);
    void bind_vertex_buffer(std::span<const std::unique_ptr<IBufferSpan>> buffer);
